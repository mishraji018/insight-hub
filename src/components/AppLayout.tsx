...
const navItems = [
  { to: "/dashboard", label: "Dashboard", icon: LayoutDashboard, roles: ['executive', 'manager', 'user'] },
  { to: "/analytics", label: "Analytics", icon: FileSpreadsheet, roles: ['executive', 'analyst', 'manager', 'user'] },
  { to: "/predictions", label: "Predictions", icon: Brain, roles: ['executive', 'analyst', 'manager', 'user'] },
  ...
  return (
    <div className="flex min-h-screen text-foreground relative">
      ...
      <div className="flex min-h-screen w-full z-10 relative">
                ...
                <div className="max-h-48 overflow-y-auto space-y-1">
                  {organisations.map(org => (
                    <button
                      ...
                      className={`w-full flex items-center gap-3 p-3 rounded-xl transition-all ${activeOrgId === org.id ? 'bg-primary/20 text-primary' : 'hover:bg-white/5 text-white/40'
                        }`}
                    >
          ...
          <nav className="flex-1 p-5 space-y-1.5 overflow-y-auto pt-0">
            {filteredNavItems.map((item) => {
              ...
              return (
                <Link
                  key={item.to}
                  to={item.to}
                  className={`flex items-center gap-3 px-4 py-3.5 rounded-2xl text-[13px] font-bold transition-all duration-300 ${active
            ...
            {isAdmin && (
              <div className="space-y-1.5 pt-4">
                ...
                <Link
                  to="/admin/dashboard"
                  className={`flex items-center gap-3 px-4 py-3.5 rounded-2xl text-[13px] font-bold transition-all duration-300 ${location.pathname === "/admin/dashboard"
                      ? "bg-primary text-primary-foreground shadow-lg shadow-primary/30 translate-x-1"
                      : "text-white/70 hover:bg-white/5 hover:text-white"
                    }`}
                >
                ...
                <Link
                  to="/admin/panel"
                  className={`flex items-center gap-3 px-4 py-3.5 rounded-2xl text-[13px] font-bold transition-all duration-300 ${location.pathname === "/admin/panel"
                ...
                <Link
                  ...
                  className={`flex items-center gap-3 px-4 py-3.5 rounded-2xl text-[13px] font-bold transition-all duration-300 ${location.pathname === "/admin/train-model"
            ...
            {/* Notification Bell and Avatars moved to sidebar vertical column */}
                <div className="px-4 py-6 space-y-6">
                  <div className="flex flex-col gap-4">
                    ...
                    <Link
                      to="/profile"
                      className={`w-full flex items-center justify-between gap-3 px-4 py-3 rounded-xl border transition-all group ${
                        location.pathname === '/profile' 
                        ? 'bg-primary/20 border-primary/50 text-white shadow-lg shadow-primary/20' 
                        : 'bg-black/40 border-white/5 hover:border-primary/30 text-white/30 hover:text-white'
                      }`}
                    >
                      <span className="text-[10px] font-black uppercase tracking-widest transition-colors">
                        My Profile
                      </span>
                      <User className={`h-4 w-4 transition-colors ${location.pathname === '/profile' ? 'text-primary' : 'text-white/40 group-hover:text-white'}`} />
                    </Link>
                  </div>
                </div>
          </nav>
        ...
        {/* Main Content */}
        <main className="flex-1 flex flex-col min-w-0 bg-transparent overflow-x-hidden relative">
          <header className="h-20 border-b border-border/40 flex items-center justify-between px-10 bg-background/30 backdrop-blur-xl sticky top-0 z-50">
            ...
            <div className="flex items-center gap-4">
              ...
              <Link
                to="/profile"
                className={`hidden sm:flex items-center gap-2 px-4 py-1.5 rounded-full ${(user?.plan_name || 'Free').toLowerCase() !== 'free' ? 'bg-amber-500/10 hover:bg-amber-500/20 border-amber-500/20 text-amber-500 shadow-amber-500/5' : 'bg-primary/10 hover:bg-primary/20 border-primary/20 text-primary shadow-primary/5'} transition-all border group shadow-sm`}
              >
              ...
              {/* Activity Dropdown */}
              <div className="relative z-[60]" ref={activityRef}>
                <button
                  onClick={() => {
                  ...
                  className={`relative p-2 rounded-xl transition-all duration-300 ${showActivity ? 'bg-primary/10 shadow-inner text-primary' : 'hover:bg-muted text-muted-foreground hover:text-foreground'
                    }`}
                >
              ...
              <div className="relative z-[60]" ref={notificationRef}>
                <button
                  onClick={() => {
                    setShowNotifications(!showNotifications);
                    setShowProfileMenu(false);
                  }}
                  className={`relative p-2 rounded-xl transition-all duration-300 ${showNotifications ? 'bg-primary/10 shadow-inner text-primary' : 'hover:bg-muted text-muted-foreground hover:text-foreground'
                    }`}
                >
                    ...
                    <div className="max-h-[400px] overflow-y-auto space-y-1">
                      {notifications.length === 0 ? (
                        ...
                        notifications.map(notif => (
                          <div
                            key={notif.id}
                            className={`p-3 rounded-xl transition-all border ${!notif.is_read ? 'bg-primary/5 border-primary/10' : 'opacity-60 grayscale-[0.5] border-transparent'
                              } hover:bg-muted group cursor-default`}
                          >
                            <div className="flex gap-3">
                              <div className={`mt-0.5 p-1.5 rounded-lg shrink-0 h-fit ${notif.type === 'security' ? 'bg-destructive/10 text-destructive' :
              ...
              <div className="relative" ref={profileRef}>
                <button
                  ...
                  className={`flex items-center gap-3 p-1.5 pr-4 rounded-xl transition-all duration-300 ${showProfileMenu ? 'bg-muted shadow-inner' : 'hover:bg-muted'
                    }`}
                >
                  <div className="h-8 w-8 rounded-lg bg-primary/20 flex items-center justify-center text-primary text-xs font-black border border-primary/20 shadow-lg shadow-primary/20 overflow-hidden">
                    {user?.avatar ? (
                      <img src={`${import.meta.env.VITE_API_BASE_URL}${user.avatar}`} alt="" className="h-full w-full object-contain" />
                    ) : (
                  ...
                  <div className="flex flex-col text-left">
                    ...
                    <span className="text-[8px] font-bold text-muted-foreground uppercase tracking-tighter">{user?.role === 'admin' ? 'ADMIN' : user?.role || 'Guest'}</span>
                  </div>
                ...
                {showProfileMenu && (
                  <div className="absolute top-full right-0 mt-3 w-56 p-2 rounded-2xl bg-popover border border-border shadow-2xl z-50 scale-in-center">
                    <div className="px-4 py-3 border-b border-border mb-2 bg-muted/50 rounded-t-xl">
                      ...
                      <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-tighter mt-0.5">{user?.role === 'admin' ? 'ADMIN' : user?.role}</p>
                    </div>
                    <Link
                      to="/profile"
