        ...
        <nav className="flex-1 p-5 space-y-1.5 overflow-y-auto pt-0">
          ...
          {isStaff && (
            <div className="space-y-1.5 pt-4">
              ...
              >
                <LayoutDashboard className={`h-4 w-4 ${location === "/admin/dashboard" ? 'text-primary-foreground' : 'text-primary/60'}`} />
                Super Dashboard
              </Link>
              <Link
                href="/admin/panel"
              ...
              >
                <Shield className={`h-4 w-4 ${location === "/admin/panel" ? 'text-black' : 'text-amber-500/60'}`} />
                Admin Panel
              </Link>
              <Link
                href="/admin/train-model"
                className={`flex items-center gap-3 px-4 py-3.5 rounded-2xl text-[13px] font-bold transition-all duration-300 ${
                  location === "/admin/train-model"
                    ? "bg-sidebar-accent text-sidebar-accent-foreground shadow-lg translate-x-1"
                    : "text-sidebar-foreground/40 hover:bg-white/5 hover:text-white"
                }`}
              >
                <Settings className={`h-4 w-4 ${location === "/admin/train-model" ? 'text-white' : 'text-white/20'}`} />
                Engine Metrics
              </Link>
            ...
            <div className="relative">
              ...
              >
                <div className="h-8 w-8 rounded-lg bg-primary/20 flex items-center justify-center text-primary text-xs font-black border border-primary/20 shadow-lg shadow-primary/20 overflow-hidden">
                  {user?.avatar ? (
                    <img src={`${process.env.NEXT_PUBLIC_API_URL}${user.avatar}`} alt="" className="h-full w-full object-cover" />
                  ) : (
              ...
              {showProfileMenu && (
                <div className="absolute top-full right-0 mt-3 w-56 p-2 rounded-2xl bg-card border border-border shadow-2xl z-30 backdrop-blur-3xl scale-in-center">
                  ...
                  <button
                    onClick={() => { logout(); navigate.push('/login'); }}
